    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['app:drama_rules:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="drama_rulesList" @selection-change="handleSelectionChange">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listDrama_rules(this.queryParams).then(response => {
        this.drama_rulesList = response.rows
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateDrama_rules(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addDrama_rules(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('app/drama_rules/export', {
        ...this.queryParams
      }, `drama_rules_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
